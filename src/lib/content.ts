export const siteConfig = {
  name: "Đất Rẫy View Thác Liêng Nung — Hồ Tà Đùng",
  phone: "034.857.9065",
  phoneRaw: "0348579065",
  zalo: "0348579065",
  address: "Phú Xuân, Đắk Nia, Gia Nghĩa, Đắk Nông",
  price: "240 triệu/sào (có thương lượng)",
  pricePerSao: 240000000,
  areaRedBook: 19445,
  areaActual: 22000,
  sao: 1000,
  mapUrl: "https://maps.app.goo.gl/ExjuQjjRqZLpdYA49",
}

export const heroContent = {
  badge: "ĐẤT VƯỜN VIEW THÁC LIÊNG NUNG – CÓ SẴN THU NHẬP",
  headline: "2.2ha Đất View Thác Liêng Nung – Phú Xuân, Gia Nghĩa, Đăk Nông",
  subheadline:
    "Cách QL28 600m • Điện 3 pha • Nước suối quanh năm • Sẵn cà phê, sầu riêng, tiêu",
  description:
    "",
  price: "💰 240 triệu/sào (có thương lượng)",
  cta: "Khám Phá Vị Trí & Hình Ảnh Thực Tế",
  trustItems: [
    { icon: "📜", text: "Sổ đỏ chính chủ 19.445m²" },
    { icon: "🌿", text: "Vườn cây đang cho thu nhập" },
    { icon: "🏞️", text: "View thác Liêng Nung + hồ" },
    { icon: "🚗", text: "Ô tô vào tận nơi" },
  ],
}

export const problemContent = {
  headline: "Bạn đang tìm đất để canh tác, tích sản hay làm farmstay?",
  painPoints: [
    {
      title: "Đất khác giá cao, pháp lý mập mờ",
      desc: "Nhiều lô đất Tây Nguyên giá bị đẩy lên cao, sổ sách không rõ ràng, tranh chấp ranh giới.",
    },
    {
      title: "Đất xa đường lớn, không có nước",
      desc: "Đường vào nhỏ hẹp, mùa khô thiếu nước tưới, không có điện 3 pha để sản xuất.",
    },
    {
      title: "Đất trống, không có thu nhập",
      desc: "Mua đất trống phải chờ lên giá, không tạo ra dòng tiền trong khi chờ đợi.",
    },
    {
      title: "Đất dốc, khó canh tác",
      desc: "Địa hình dốc khó làm đường, khó tưới tiêu, chi phí san lấp cao.",
    },
  ],
}

export const credibilityContent = {
  headline: "Tại sao nên chọn lô đất này?",
  items: [
    {
      icon: "📜",
      title: "Sổ đỏ chính chủ - 19.445m²",
      desc: "Diện tích trên sổ gần 2ha, thực tế ~2.2ha. Pháp lý rõ ràng, sang tên nhanh chóng.",
    },
    {
      icon: "🌿",
      title: "Có sẵn vườn cây ăn trái",
      desc: "Cà phê 1ha (4 tấn nhân/năm), sầu riêng 100 cây (3 tấn/năm) + 200 cây con đang lớn, hồ tiêu 0.5 sào (5 tạ/năm).",
    },
    {
      icon: "⚡",
      title: "Điện 3 pha - Nước quanh năm",
      desc: "Điện 3 pha + bơm 3 pha. Nước suối Đắk Nia không bao giờ cạn, tưới tiêu thoải mái.",
    },
    {
      icon: "🛣️",
      title: "Đường 5-8m, cách QL28 chỉ 600m",
      desc: "Đường vào rộng rãi, ô tô vào tận nơi. Cách quốc lộ chỉ vài trăm mét.",
    },
    {
      icon: "🥾",
      title: "Cung trekking 2km xuống thác",
      desc: "Cung đường xuyên rừng 2km từ lô đất xuống thác Liêng Nung — trải nghiệm độc đáo cho khách farmstay.",
    },
    {
      icon: "🏡",
      title: "Phù hợp đa năng",
      desc: "Canh tác – tích sản – farmstay đều được. Địa hình thoải, dễ quy hoạch.",
    },
  ],
}

export const storyContent = {
  headline: "Trước và Sau khi sở hữu lô đất",
  before: {
    label: "Trước đây",
    points: [
      "Tìm đất Tây Nguyên: giá cao, xa đường, không có nước tưới",
      "Mua đất trống: không có thu nhập, phải chờ tăng giá",
      "Đầu tư vào đất dốc: tốn chi phí san lấp, khó canh tác",
      "Pháp lý mập mờ: sổ sách không rõ, tranh chấp ranh giới",
    ],
  },
  after: {
    label: "Sau khi sở hữu lô đất này",
    points: [
      "Có ngay vườn cây đang cho thu hoạch: cà phê 4 tấn, sầu riêng 3 tấn, tiêu 5 tạ/năm",
      "Vị trí đắc địa: view thác Liêng Nung & Hồ Đắk Nia, gần Hồ Tà Đùng, tiềm năng farmstay rất lớn",
      "Cơ sở hạ tầng hoàn chỉnh: đường 5-8m, điện 3 pha, nước suối quanh năm",
      "Địa hình thoải, dễ quy hoạch: làm vườn, làm farmstay, tích sản đều phù hợp",
    ],
  },
}

export const galleryContent = {
  headline: "Hình ảnh & Video thực tế lô đất",
  images: [
    { url: "images/daknong-1.jpg?t=1782683112", alt: "Toàn cảnh view thác Liêng Nung và Hồ Đắk Nia từ lô đất", caption: "View trực diện thác Liêng Nung & Hồ Đắk Nia" },
    { url: "images/daknong-2.jpg?t=1782683112", alt: "Vườn cây ăn trái đang phát triển trên đất", caption: "Vườn cây xanh tốt — đất đai màu mỡ" },
    { url: "images/daknong-3.jpg?t=1782683112", alt: "Cảnh quan lô đất rộng rãi thoáng đãng", caption: "Không gian rộng ~2.2ha — thoải, dễ quy hoạch" },
    { url: "images/daknong-4.jpg?t=1782683112", alt: "Toàn cảnh lô đất từ flycam", caption: "Flycam tổng quan — địa hình thoải, view hồ" },
    { url: "images/daknong-5.jpg?t=1782683112", alt: "Lô đất nhìn từ trên cao với view Hồ Đắk Nia", caption: "View flycam — Hồ Đắk Nia & thác Liêng Nung" },
    { url: "images/daknong-6.jpg?t=1782683112", alt: "Sổ đỏ chính chủ diện tích 19.445m²", caption: "Sổ đỏ chính chủ — sang tên nhanh chóng" },
    { url: "images/daknong-7.jpg?t=1782683112", alt: "Vườn cà phê đang cho thu hoạch", caption: "Cà phê 1ha — 4 tấn nhân/năm" },
    { url: "images/daknong-8.jpg?t=1782683112", alt: "Vườn sầu riêng 100 cây đang thu hoạch", caption: "Sầu riêng 100 cây — 3 tấn/năm" },
    { url: "images/daknong-9.jpg?t=1782683112", alt: "Đường vào lô đất rộng 5-8m", caption: "Đường 5-8m, ô tô vào tận nơi" },
    { url: "images/daknong-10.jpg?t=1782683112", alt: "Vườn hồ tiêu 0.5 sào", caption: "Hồ tiêu 0.5 sào — 5 tạ/năm" },
    { url: "images/daknong-11.jpg?t=1782683112", alt: "Toàn cảnh khu vực từ flycam", caption: "Flycam — tổng quan vị trí đắc địa" },
    { url: "images/daknong-12.jpg?t=1782683112", alt: "Vườn cây xanh tốt trên lô đất", caption: "Vườn cây xanh tốt — đất đai màu mỡ" },
  ],
  videos: [
    { url: "videos/hero-bg.mp4?t=1782683112", poster: "images/daknong-4.jpg?t=1782683112", alt: "Toàn cảnh lô đất từ flycam — view thác Liêng Nung & Hồ Đắk Nia", caption: "Flycam toàn cảnh — view thác Liêng Nung & Hồ Đắk Nia" },
    { url: "images/hero-video.mp4?t=1782683112", poster: "images/daknong-9.jpg?t=1782683112", alt: "Đường 5-8m, ô tô vào tận nơi", caption: "Đường 5-8m, ô tô vào tận nơi — video thực tế" },
    { url: "images/daknong-video-1.mp4?t=1782683112", poster: "images/daknong-7.jpg?t=1782683112", alt: "Vườn cà phê và sầu riêng trên đất", caption: "Vườn cây xanh tốt — video tổng quan" },
    { url: "images/daknong-video-2.mp4?t=1782683112", poster: "images/daknong-1.jpg?t=1782683112", alt: "Không gian lô đất rộng rãi thoáng đãng", caption: "Không gian lô đất — video thực tế" },
  ],
}

export const locationContent = {
  headline: "Vị trí đắc địa — View thác Liêng Nung & Hồ Đắk Nia",
  subheadline: "Phú Xuân, Đắk Nia, Gia Nghĩa, Đắk Nông",
  description: "Lô đất nằm tại Phú Xuân, Đắk Nia, cách Quốc lộ 28 chỉ 600m. View trực diện thác Liêng Nung và gần Hồ Đắk Nia. Khu vực giáp ranh các điểm du lịch nổi tiếng như Hồ Tà Đùng (~27km) — \"Hạ Long trên cạn\" của Tây Nguyên, Eva Village, Hama Village.",
  mapUrl: "https://maps.app.goo.gl/ExjuQjjRqZLpdYA49",
  nearbyRoads: [
    { name: "Quốc lộ 28", distance: "600m", desc: "Trục đường chính kết nối Gia Nghĩa - Bình Thuận" },
    { name: "Trung tâm Gia Nghĩa", distance: "~8 km", desc: "Trung tâm tỉnh Đắk Nông" },
    { name: "DT 684", distance: "~2 km", desc: "Đường tỉnh kết nối các xã" },
  ],
  amenities: [
    { name: "Thác Liêng Nung", distance: "2 km (trekking)", time: "trekking 2km", icon: "🏔️", desc: "Thác nước hùng vĩ, điểm du lịch nổi tiếng" },
    { name: "Hồ Tà Đùng", distance: "~27 km", time: "30 phút", icon: "🏞️", desc: "Khu du lịch sinh thái nổi tiếng — \"Hạ Long trên cạn\" với 40 đảo đá vôi" },
    { name: "Eva Village", distance: "~4 km", time: "5 phút", icon: "🏡", desc: "Khu du lịch sinh thái nổi tiếng Đắk Nông" },
    { name: "Hama Village", distance: "~1 km", time: "2 phút", icon: "🏡", desc: "Khu nghỉ dưỡng homestay view hồ Đắk Nia" },
    { name: "Trung tâm Gia Nghĩa", distance: "8 km", time: "10 phút", icon: "🏙️", desc: "Trung tâm hành chính tỉnh Đắk Nông" },
    { name: "Chợ Đắk Nia", distance: "~3 km", time: "5 phút", icon: "🛒", desc: "Chợ địa phương, đầy đủ nhu yếu phẩm" },
    { name: "QL28", distance: "600m", time: "1 phút", icon: "🛣️", desc: "Kết nối giao thông thuận tiện" },
    { name: "Đà Lạt", distance: "~130 km", time: "2.5 giờ", icon: "🏞️", desc: "Tour du lịch kết nối Đà Lạt - Đắk Nông" },
  ],
}

export const investmentComparison = {
  badge: "SO SÁNH TIỀM NĂNG",
  headline: "Gửi ngân hàng, mua vàng — hay đầu tư đất vườn có sẵn thu nhập?",
  items: [
    {
      label: "Gửi ngân hàng",
      rate: "~4.5%/năm",
      monthly: "~8.6 triệu/tháng (với 4.5 tỷ)",
      note: "Tiền mất giá theo lạm phát, không có tài sản",
      color: "red",
    },
    {
      label: "Mua vàng",
      rate: "Biến động",
      monthly: "Không sinh lời hàng tháng",
      note: "Không tạo ra dòng tiền, bất tiện khi bán",
      color: "red",
    },
    {
      label: "Mua đất vườn này",
      rate: "Thu nhập 100+ triệu/năm",
      monthly: "Cà phê + sầu riêng + tiêu",
      note: "Có đất + vườn cây + view thác + trekking, tài sản tăng giá, vào là có thu nhập ngay",
      color: "emerald",
    },
  ],
    conclusion: "Đầu tư đất vườn: vừa giữ tiền, vừa có thu nhập hàng năm ổn định. View thác Liêng Nung cực kỳ hiếm — tiềm năng phát triển farmstay rất lớn.",
}

export const expansionContent = {
  detail: "Hiện tại đã có 1ha cà phê (4 tấn nhân/năm), 100 cây sầu riêng thu hoạch (3 tấn/năm) + 200 cây con đang lớn, và 0.5 sào hồ tiêu (5 tạ/năm). Địa hình thoải rộng ~2.2ha, có thể mở rộng thêm farmstay, homestay hoặc trồng thêm cây ăn trái.",
  current: "Cà phê 4 tấn/năm + Sầu riêng 3 tấn/năm + Tiêu 5 tạ/năm + 200 cây sầu riêng con",
  potential: "Tiềm năng Farmstay + Mở rộng vườn",
}

export const routeContent = {
  headline: "Lộ trình đến lô đất",
  description: "Từ Quốc lộ 28 vào tận lô đất",
  steps: [
    { position: "Quốc lộ 28 (ngã ba Đắk Nia)", icon: "🛣️" },
    { position: "Rẽ vào đường Phú Xuân 600m", icon: "↪️" },
    { position: "Đường đất đỏ 5-8m", icon: "🚗" },
    { position: "Đến lô đất — view thác Liêng Nung & Hồ Đắk Nia", icon: "🏠" },
  ],
}

export const leadFormContent = {
  headline: "Để Lại Số Điện Thoại — Tôi Sẽ Tư Vấn Ngay",
  subheadline: "Gọi lại cho anh/chị trong vòng 30 phút, giải đáp mọi thắc mắc về lô đất",
  cta: "Gửi Thông Tin",
  phoneCta: "Gọi Ngay: 034.857.9065",
}

export const faqContent = {
  headline: "Câu hỏi thường gặp",
  items: [
    {
      q: "Pháp lý lô đất có vấn đề gì không?",
      a: "Sổ đỏ chính chủ, diện tích 19.445m². Pháp lý rõ ràng, không tranh chấp. Anh/chị có thể kiểm tra thông tin tại UBND xã Đắk Nia trước khi giao dịch.",
    },
    {
      q: "Đường vào có khó không? Xe gì vào được?",
      a: "Đường đất đỏ rộng 5-8m, cách QL28 chỉ 600m. Ô tô vào tận nơi, kể cả mùa mưa. Địa hình thoải, xe tải nhẹ vào được.",
    },
    {
      q: "Nước tưới có đủ không? Mùa khô thiếu nước không?",
      a: "Suối Đắk Nia chảy quanh năm, không bao giờ cạn. Có máy bơm 3 pha công suất lớn, đảm bảo tưới tiêu cho toàn bộ diện tích.",
    },
    {
      q: "Thu nhập từ vườn cây có ổn định không?",
      a: "Cà phê 1ha cho 4 tấn nhân/năm, sầu riêng 100 cây cho 3 tấn/năm + 200 cây con đang lớn, hồ tiêu 0.5 sào cho 5 tạ/năm. Đây là những cây trồng chủ lực của Tây Nguyên, đầu ra ổn định.",
    },
    {
      q: "Có thể làm farmstay được không?",
      a: "View trực diện thác Liêng Nung và Hồ Đắk Nia — một trong những vị trí đẹp nhất khu vực. Địa hình thoải, có cung trekking 2km xuyên rừng xuống thác. Gần các khu du lịch Hồ Tà Đùng (~27km), Eva Village, Hama Village — rất phù hợp làm farmstay, homestay.",
    },
    {
      q: "Giá có thương lượng không? Thủ tục mua bán thế nào?",
      a: "Giá 240 triệu/sào (có thương lượng). Đặt cọc → Công chứng hợp đồng → Nộp hồ sơ → Đóng thuế → Nhận sổ mới (15-30 ngày). Hoa hồng đầy đủ cho môi giới.",
    },
    {
      q: "Có lao động tại chỗ để thuê làm vườn không?",
      a: "Có. Khu vực Phú Xuân, Đắk Nia có lao động bản địa dồi dào, quen với việc canh tác cà phê, sầu riêng, hồ tiêu. Chi phí nhân công thấp hơn so với các tỉnh lân cận.",
    },
    {
      q: "Logistics vận chuyển nông sản có thuận lợi không?",
      a: "Cách QL28 chỉ 600m, kết nối trực tiếp với các tuyến cao tốc hướng về cảng Cát Lái và sân bay quốc tế (khoảng 4-5 giờ). Đường đất đỏ 5-8m, xe tải nhẹ vào tận nơi, thuận tiện cho vận chuyển nông sản xuất khẩu.",
    },
  ],
}

export const motto = "Anh em môi giới có khách cứ kết nối, HH môi giới đầy đủ"
